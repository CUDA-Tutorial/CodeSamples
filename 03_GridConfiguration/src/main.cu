...
{
    std::cout << "Small grid: \n";
    // Configure the grid and block dimensions via
    // built-in struct dim3 (X,Y,Z)
    dim3 gridSize_small{ 1, 1, 1 };
    dim3 blockSize_small{ 4, 4, 1 };
    // Launch kernel with custom grid
    PrintIDs<<<gridSize_small, blockSize_small>>>();
    // We need to synchronize here to have the 
    // GPU and CPU printouts in the correct order
